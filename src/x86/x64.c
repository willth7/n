...
void x86_64_reg_print(uint8_t* bin) {
	...
	printf("    %02x %02x %02x %02x:%02x %02x:%02x:%02x  \n", bin[55], bin[54], bin[53], bin[52], bin[51], bin[50], bin[49], bin[48]);
	printf("                  dh:dl");
	printf("                        :sil \n");
	printf("rbx        :ebx  :bx   ");
	printf("    rdi        :edi  :di     \n");
...
void x86_64_print_stack(uint8_t* bin, uint64_t* ip, uint64_t bn) {
	for (uint8_t i = 0; (i < 8) && (i < (bn - *ip)); i++) {
		printf("%02x ", bin[*ip + i]);
	}
	if ((bn - *ip) < 8) {
		for (uint8_t i = 8 - (bn - *ip); i > 0; i--) {
			printf("   ");
		}
	}
	printf("                     ");
	for (uint8_t i = 0; (i < 8) && (i < (bn - *ip)); i++) {
		if (bin[*ip + i] >= 32 && bin[*ip + i] < 127) {
			printf("%c", bin[*ip + i]);
		}
		else if (bin[*ip + i] == 0) {
			printf("\\0");
...
uint8_t x86_64_dec_mvp(uint8_t* bin, uint64_t* bn, uint64_t* addr, uint8_t op, int8_t* mn, uint8_t lga, uint8_t lgo, uint8_t rex, uint8_t rx0, uint8_t rx1, uint8_t rx2, uint8_t rx3) {
	if ((bin[*bn] & 248) == op) {
		...
		*bn = *bn + 1;
		if (rex) {
			printf("            %s %s, %u", mn, x86_64_dec_r8x(reg), k);
		}
		else {
...
void x86_64_inc_add_imm_64(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t imm) {
	if (dst >= bn) {
		...
		cf = ((bin[dst + 4] + car) & 256) >> 8;
		pf = x86_64_parity_flag(bin[dst + 4] + car, pf);
		zf = (!(bin[dst + 4] + car) && zf) << 6;
		bin[dst + 4] = bin[dst + 4] + car;
		car = cf;
		
		cf = ((bin[dst + 5] + car) & 256) >> 8;
		pf = x86_64_parity_flag(bin[dst + 5] + car, pf);
		zf = (!(bin[dst + 5] + car) && zf) << 6;
		bin[dst + 5] = bin[dst + 5] + car;
		car = cf;
		
		cf = ((bin[dst + 6] + car) & 256) >> 8;
		pf = x86_64_parity_flag(bin[dst + 6] + car, pf);
		zf = (!(bin[dst + 6] + car) && zf) << 6;
		bin[dst + 6] = bin[dst + 6] + car;
		car = cf;
		
		cf = ((bin[dst + 7] + car) & 256) >> 8;
		pf = x86_64_parity_flag(bin[dst + 7] + car, pf);
		zf = (!(bin[dst + 7] + car) && zf) << 6;
		uint8_t sf = bin[dst + 7] & 128;
		uint8_t of = 0;
		bin[dst + 7] = bin[dst + 7] + car;
		car = cf;
...
void x86_64_inc_sbb_imm_64(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t imm) {
	if (dst >= bn) {
		...
		cf = ((bin[dst + 4] + 255 + bor) & 256) >> 8;
		pf = x86_64_parity_flag(bin[dst + 4] + 255 + bor, pf);
		zf = (!((bin[dst + 4] + 255 + bor) & 255) && zf) << 6;
		bin[dst + 4] = bin[dst + 4] + 255 + bor;
		bor = cf;
		
		cf = ((bin[dst + 5] + 255 + bor) & 256) >> 8;
		pf = x86_64_parity_flag(bin[dst + 5] + 255 + bor, pf);
		zf = (!((bin[dst + 5] + 255 + bor) & 255) && zf) << 6;
		bin[dst + 5] = bin[dst + 5] + 255 + bor;
		bor = cf;
		
		cf = ((bin[dst + 6] + 255 + bor) & 256) >> 8;
		pf = x86_64_parity_flag(bin[dst + 6] + 255 + bor, pf);
		zf = (!((bin[dst + 6] + 255 + bor) & 255) && zf) << 6;
		bin[dst + 6] = bin[dst + 6] + 255 + bor;
		bor = cf;
		
		cf = ((bin[dst + 7] + 255 + bor) & 256) >> 8;
		pf = x86_64_parity_flag(bin[dst + 7] + 255 + bor, pf);
		zf = (!((bin[dst + 7] + 255 + bor) & 255) && zf) << 6;
		uint8_t sf = (bin[dst + 7] + 255 + bor) & 128;
		uint8_t of = ((bin[dst + 7] & 128) ^ 1) << 3;
		bin[dst + 7] = bin[dst + 7] + 255 + bor;
...
void x86_64_inc_mov_imm_8(uint8_t* bin, uint64_t bn, uint64_t dst, uint8_t imm) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_mov_imm_16(uint8_t* bin, uint64_t bn, uint64_t dst, uint16_t imm) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_mov_imm_32(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t imm) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_mov_imm_64(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t imm) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_mov_imm_64_mvp(uint8_t* bin, uint64_t bn, uint64_t dst, uint64_t imm) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_push_reg_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
...
void x86_64_inc_push_reg_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	uint64_t sp = x86_64_reg_get_sp(bin) + 138;
	if ((sp - 8) >= bn || (sp - 8) < 138) {
		printf("[error] illegal memory access\n");
	}
...
void x86_64_inc_pop_reg_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	uint64_t sp = x86_64_reg_get_sp(bin) + 138;
	if (sp >= bn || sp < 138) {
		printf("[error] illegal memory access\n");
	}
...
void x86_64_inc_pop_reg_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	uint64_t sp = x86_64_reg_get_sp(bin) + 138;
	if (sp >= bn || sp < 138) {
		printf("[error] illegal memory access\n");
	}
...
void x86_64_inc_rol_imm_8(uint8_t* bin, uint64_t bn, uint64_t dst, uint8_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rol_imm_16(uint8_t* bin, uint64_t bn, uint64_t dst, uint16_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
		...
		for (; k > 0; k--) {
			cf = bin[dst + 1] >> 7;
			bin[dst + 1] = (bin[dst + 1] << 1) | (bin[dst] >> 7);
			bin[dst] = (bin[dst] << 1) | cf;
		}
...
void x86_64_inc_rol_imm_32(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
		...
		for (; k > 0; k--) {
			cf = bin[dst + 3] >> 7;
			bin[dst + 3] = (bin[dst + 3] << 1) | (bin[dst + 2] >> 7);
			bin[dst + 2] = (bin[dst + 2] << 1) | (bin[dst + 1] >> 7);
			bin[dst + 1] = (bin[dst + 1] << 1) | (bin[dst] >> 7);
			bin[dst] = (bin[dst] << 1) | cf;
		}
...
void x86_64_inc_rol_imm_64(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
		...
		for (; k > 0; k--) {
			cf = bin[dst + 7] >> 7;
			bin[dst + 7] = (bin[dst + 7] << 1) | (bin[dst + 6] >> 7);
			bin[dst + 6] = (bin[dst + 6] << 1) | (bin[dst + 5] >> 7);
			bin[dst + 5] = (bin[dst + 5] << 1) | (bin[dst + 4] >> 7);
			bin[dst + 4] = (bin[dst + 4] << 1) | (bin[dst + 3] >> 7);
			bin[dst + 3] = (bin[dst + 3] << 1) | (bin[dst + 2] >> 7);
			bin[dst + 2] = (bin[dst + 2] << 1) | (bin[dst + 1] >> 7);
			bin[dst + 1] = (bin[dst + 1] << 1) | (bin[dst] >> 7);
			bin[dst] = (bin[dst] << 1) | cf;
...
void x86_64_inc_rol_k1_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rol_k1_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rol_k1_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rol_k1_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rol_cl_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rol_cl_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rol_cl_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rol_cl_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_ror_imm_8(uint8_t* bin, uint64_t bn, uint64_t dst, uint8_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_ror_imm_16(uint8_t* bin, uint64_t bn, uint64_t dst, uint16_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
		...
		for (; k > 0; k--) {
			cf = bin[dst] & 1;
			bin[dst] = (bin[dst] >> 1) | (bin[dst + 1] << 7);
			bin[dst + 1] = (bin[dst + 1] >> 1) | (cf << 7);
		}
...
void x86_64_inc_ror_imm_32(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
		...
		for (; k > 0; k--) {
			cf = bin[dst] & 1;
			bin[dst] = (bin[dst] >> 1) | (bin[dst + 1] << 7);
			bin[dst + 1] = (bin[dst + 1] >> 1) | (bin[dst + 2] << 7);
			bin[dst + 2] = (bin[dst + 2] >> 1) | (bin[dst + 3] << 7);
			bin[dst + 3] = (bin[dst + 3] >> 1) | (cf << 7);
		}
...
void x86_64_inc_ror_imm_64(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
		...
		for (; k > 0; k--) {
			...
			bin[dst + 1] = (bin[dst + 1] >> 1) | (bin[dst + 2] << 7);
			bin[dst + 2] = (bin[dst + 2] >> 1) | (bin[dst + 3] << 7);
			bin[dst + 3] = (bin[dst + 3] >> 1) | (bin[dst + 4] << 7);
			bin[dst + 4] = (bin[dst + 4] >> 1) | (bin[dst + 5] << 7);
			bin[dst + 5] = (bin[dst + 5] >> 1) | (bin[dst + 6] << 7);
			bin[dst + 6] = (bin[dst + 6] >> 1) | (bin[dst + 7] << 7);
			bin[dst + 7] = (bin[dst + 7] >> 1) | (cf << 7);
		}
...
void x86_64_inc_ror_k1_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_ror_k1_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_ror_k1_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_ror_k1_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_ror_cl_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_ror_cl_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_ror_cl_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_ror_cl_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcl_imm_8(uint8_t* bin, uint64_t bn, uint64_t dst, uint8_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcl_imm_16(uint8_t* bin, uint64_t bn, uint64_t dst, uint16_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcl_imm_32(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcl_imm_64(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcl_k1_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcl_k1_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcl_k1_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcl_k1_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcl_cl_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcl_cl_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcl_cl_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcl_cl_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcr_imm_8(uint8_t* bin, uint64_t bn, uint64_t dst, uint8_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcr_imm_16(uint8_t* bin, uint64_t bn, uint64_t dst, uint16_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcr_imm_32(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcr_imm_64(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcr_k1_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcr_k1_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcr_k1_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcr_k1_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcr_cl_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcr_cl_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcr_cl_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_rcr_cl_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shl_imm_8(uint8_t* bin, uint64_t bn, uint64_t dst, uint8_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shl_imm_16(uint8_t* bin, uint64_t bn, uint64_t dst, uint16_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shl_imm_32(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shl_imm_64(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shl_k1_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shl_k1_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shl_k1_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shl_k1_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shl_cl_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shl_cl_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shl_cl_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shl_cl_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shr_imm_8(uint8_t* bin, uint64_t bn, uint64_t dst, uint8_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shr_imm_16(uint8_t* bin, uint64_t bn, uint64_t dst, uint16_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shr_imm_32(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shr_imm_64(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shr_k1_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shr_k1_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shr_k1_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shr_k1_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shr_cl_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shr_cl_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shr_cl_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_shr_cl_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sal_imm_8(uint8_t* bin, uint64_t bn, uint64_t dst, uint8_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sal_imm_16(uint8_t* bin, uint64_t bn, uint64_t dst, uint16_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sal_imm_32(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sal_imm_64(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sal_k1_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sal_k1_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sal_k1_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sal_k1_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sal_cl_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sal_cl_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sal_cl_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sal_cl_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sar_imm_8(uint8_t* bin, uint64_t bn, uint64_t dst, uint8_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sar_imm_16(uint8_t* bin, uint64_t bn, uint64_t dst, uint16_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sar_imm_32(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sar_imm_64(uint8_t* bin, uint64_t bn, uint64_t dst, uint32_t k) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sar_k1_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sar_k1_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sar_k1_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sar_k1_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sar_cl_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sar_cl_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sar_cl_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_sar_cl_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_not_reg_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_not_reg_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_not_reg_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_not_reg_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_neg_reg_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
		...
		uint8_t cf = ((~bin[dst] + 1) & 256) >> 8;
		uint8_t pf = x86_64_parity_flag(~bin[dst] + 1, 1);
		uint8_t af = ((~bin[dst] & 15) + 1) & 16;
		uint8_t zf = !(~bin[dst] + 1) << 6;
		uint8_t sf = (~bin[dst] + 1) & 128;
		uint8_t of = !!(bin[dst] & 128) << 3;
		bin[dst] = ~bin[dst] + 1;
...
void x86_64_inc_neg_reg_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
		...
		uint8_t cf = ((~bin[dst] + 1) & 256) >> 8;
		uint8_t pf = x86_64_parity_flag(~bin[dst] + 1, 1);
		uint8_t af = ((bin[dst] & 15) + (~bin[dst] & 15) + 1) & 16;
		uint8_t zf = !(~bin[dst] + 1) << 6;
		bin[dst] = ~bin[dst] + 1;
		uint8_t bor = !cf;
		
		cf = ((~bin[dst + 1] + bor) & 256) >> 8;
		pf = x86_64_parity_flag(~bin[dst + 1] + bor, pf);
		zf = (!(~bin[dst + 1] + bor) && zf) << 6;
		uint8_t sf = (~bin[dst + 1] + bor) & 128;
		uint8_t of = !!(bin[dst + 1] & 128) << 3;
		bin[dst + 1] = ~bin[dst + 1] + bor;
...
void x86_64_inc_neg_reg_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
		...
		cf = ((~bin[dst + 2] + bor) & 256) >> 8;
		pf = x86_64_parity_flag(~bin[dst + 2] + bor, pf);
		zf = (!(~bin[dst + 2] + bor) && zf) << 6;
		bin[dst + 2] = ~bin[dst + 2] + bor;
		bor = !cf;
		
		cf = ((~bin[dst + 3] + bor) & 256) >> 8;
		pf = x86_64_parity_flag(~bin[dst + 3] + bor, pf);
		zf = (!(~bin[dst + 3] + bor) && zf) << 6;
		uint8_t sf = (~bin[dst + 3] + bor) & 128;
		uint8_t of = !!(bin[dst + 3] & 128) << 3;
		bin[dst + 3] = ~bin[dst + 3] + bor;
...
void x86_64_inc_neg_reg_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
		...
		cf = ((~bin[dst + 4] + bor) & 256) >> 8;
		pf = x86_64_parity_flag(~bin[dst + 4] + bor, pf);
		zf = (!(~bin[dst + 4] + bor) && zf) << 6;
		bin[dst + 4] = ~bin[dst + 4] + bor;
		bor = !cf;
		
		cf = ((~bin[dst + 5] + bor) & 256) >> 8;
		pf = x86_64_parity_flag(~bin[dst + 5] + bor, pf);
		zf = (!(~bin[dst + 5] + bor) && zf) << 6;
		bin[dst + 5] = ~bin[dst + 5] + bor;
		bor = !cf;
		
		cf = ((~bin[dst + 6] + bor) & 256) >> 8;
		pf = x86_64_parity_flag(~bin[dst + 6] + bor, pf);
		zf = (!(~bin[dst + 6] + bor) && zf) << 6;
		bin[dst + 6] = ~bin[dst + 6] + bor;
		bor = !cf;
		
		cf = ((~bin[dst + 7] + bor) & 256) >> 8;
		pf = x86_64_parity_flag(~bin[dst + 7] + bor, pf);
		zf = (!(~bin[dst + 7] + bor) && zf) << 6;
		uint8_t sf = (~bin[dst + 7] + bor) & 128;
		uint8_t of = !!(bin[dst + 7] & 128) << 3;
		bin[dst + 7] = ~bin[dst + 7] + bor;
...
void x86_64_inc_inc_reg_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_inc_reg_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
		...
		cf = ((bin[dst + 1] + car) & 256) >> 8;
		pf = x86_64_parity_flag(bin[dst + 1] + car, pf);
		zf = (!(bin[dst + 1] + car) && zf) << 6;
		uint8_t sf = (bin[dst + 1] + car) & 128;
		uint8_t of = 0;
		bin[dst + 1] = bin[dst + 1] + car;
...
void x86_64_inc_inc_reg_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
		...
		cf = ((bin[dst + 2] + car) & 256) >> 8;
		pf = x86_64_parity_flag(bin[dst + 2] + car, pf);
		zf = (!(bin[dst + 2] + car) && zf) << 6;
		bin[dst + 2] = bin[dst + 2] + car;
		car = cf;
		
		cf = ((bin[dst + 3] + car) & 256) >> 8;
		pf = x86_64_parity_flag(bin[dst + 3] + car, pf);
		zf = (!(bin[dst + 3] + car) && zf) << 6;
		uint8_t sf = (bin[dst + 3] + car) & 128;
		uint8_t of = 0;
		bin[dst + 3] = bin[dst + 3] + car;
...
void x86_64_inc_inc_reg_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_dec_reg_8(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
		...
		uint8_t cf = ((bin[dst] + 255) & 256) >> 8;
		uint8_t pf = x86_64_parity_flag(bin[dst] + 255, 1);
		uint8_t af = ((bin[dst] & 15) + 255) & 16;
		uint8_t zf = !(bin[dst] + 255) << 6;
		uint8_t sf = (bin[dst] + 255) & 128;
		uint8_t of = ((bin[dst] & 128) ^ 128) << 3;
		bin[dst] = bin[dst] + 255;
...
void x86_64_inc_dec_reg_16(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
		...
		cf = ((bin[dst + 1] + 255 + bor) & 256) >> 8;
		pf = x86_64_parity_flag(bin[dst + 1] + 255 + bor, pf);
		zf = !(bin[dst + 1] + 255 && zf) << 6;
		uint8_t sf = (bin[dst + 1] + 255 + bor) & 128;
		uint8_t of = ((bin[dst + 1] & 128) ^ 128) << 3;
		bin[dst + 1] = bin[dst + 1] + 255 + bor;
...
void x86_64_inc_dec_reg_32(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
		...
		cf = ((bin[dst + 1] + 255 + bor) & 256) >> 8;
		pf = x86_64_parity_flag(bin[dst + 1] + 255 + bor, pf);
		zf = (!(bin[dst + 1] + 255 + bor) && zf) << 6;
		bin[dst + 1] = bin[dst + 1] + 255 + bor;
		bor = !cf;
		
		cf = ((bin[dst + 2] + 255 + bor) & 256) >> 8;
		pf = x86_64_parity_flag(bin[dst + 2] + 255 + bor, pf);
		zf = (!(bin[dst + 2] + 255 + bor) && zf) << 6;
		bin[dst + 2] = bin[dst + 2] + 255 + bor;
		bor = !cf;
		
		cf = ((bin[dst + 3] + 255 + bor) & 256) >> 8;
		pf = x86_64_parity_flag(bin[dst + 3] + 255 + bor, pf);
		zf = (!(bin[dst + 3] + 255 + bor) && zf) << 6;
		uint8_t sf = (bin[dst + 3] + 255 + bor) & 128;
		uint8_t of = ((bin[dst + 3] & 128) ^ 128) << 3;
		bin[dst + 3] = bin[dst + 3] + 255 + bor;
...
void x86_64_inc_dec_reg_64(uint8_t* bin, uint64_t bn, uint64_t dst) {
	if (dst >= bn) {
		printf("[error] illegal memory access\n");
	}
	else {
...
void x86_64_inc_call_reg_16(uint8_t* bin, uint64_t bn, uint64_t ip, uint64_t src) { 
	uint64_t sp = x86_64_reg_get_sp(bin) + 138;
	if (src >= bn) {
		printf("[error] illegal memory access\n");
	}
	uint64_t k = bin[src] + (bin[src + 1] << 8);
	if ((ip - k) >= bn || (ip - k) < 138 || (sp - 8) >= bn || (sp - 8) < 138) {
		printf("[error] illegal memory access\n");
...
void x86_64_inc_call_reg_64(uint8_t* bin, uint64_t bn, uint64_t ip, uint64_t src) { 
	uint64_t sp = x86_64_reg_get_sp(bin) + 138;
	if (src >= bn) {
		printf("[error] illegal memory access\n");
	}
	uint64_t k = bin[src] + (bin[src + 1] << 8) + (bin[src + 2] << 16) + (bin[src + 3] << 24) + (bin[src + 4] << 32) + (bin[src + 5] << 40) + (bin[src + 6] << 48) + (bin[src + 7] << 56) + 138;
	if (k >= bn || k < 138 || (sp - 8) >= bn || (sp - 8) < 138) {
		printf("[error] illegal memory access\n");
		x86_64_reg_set_ip(bin, bn, ip);
	}
	else {
		...
		bin[sp + 6] = ip >> 48;
		bin[sp + 7] = ip >> 56;
		x86_64_reg_set_ip(bin, bn, k);
		x86_64_reg_set_sp(bin, sp);
	}
}

void x86_64_inc_jmp_reg_16(uint8_t* bin, uint64_t bn, uint64_t ip, uint64_t src) { 
	if (src >= bn) {
		printf("[error] illegal memory access\n");
	}
	uint64_t k = bin[src] + (bin[src + 1] << 8);
...
void x86_64_inc_jmp_reg_64(uint8_t* bin, uint64_t bn, uint64_t ip, uint64_t src) { 
	if (src >= bn) {
		printf("[error] illegal memory access\n");
	}
	uint64_t k = bin[src] + (bin[src + 1] << 8) + (bin[src + 2] << 16) + (bin[src + 3] << 24) + (bin[src + 4] << 32) + (bin[src + 5] << 40) + (bin[src + 6] << 48) + (bin[src + 7] << 56) + 138;
...
void x86_64_inc_jo_s(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) {
...
void x86_64_inc_jo_u(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t of = bin[137] & 8;
	if (of) {
		if ((ip + k) >= bn || (ip + k) < 138) {
			printf("[error] illegal memory access\n");
...
void x86_64_inc_jno_s(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t of = bin[137] & 8;
	if (!of) {
		if ((ip - k) >= bn || (ip - k) < 138) {
			printf("[error] illegal memory access\n");
...
void x86_64_inc_jno_u(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t of = bin[137] & 8;
	if (!of) {
		if ((ip + k) >= bn || (ip + k) < 138) {
			printf("[error] illegal memory access\n");
...
void x86_64_inc_jc_s(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) {
...
void x86_64_inc_jc_u(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) {
	uint8_t cf = bin[136] & 1;
	if (cf) {
		if ((ip + k) >= bn || (ip + k) < 138) {
			printf("[error] illegal memory access\n");
...
void x86_64_inc_jnc_s(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) {
	uint8_t cf = bin[136] & 1;
	if (!cf) {
		if ((ip - k) >= bn || (ip - k) < 138) {
			printf("[error] illegal memory access\n");
...
void x86_64_inc_jnc_u(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) {
	uint8_t cf = bin[136] & 1;
	if (!cf) {
		if ((ip + k) >= bn || (ip + k) < 138) {
			printf("[error] illegal memory access\n");
...
void x86_64_inc_je_s(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
...
void x86_64_inc_je_u(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t zf = bin[136] & 64;
	if (zf) {
		if ((ip + k) >= bn || (ip + k) < 138) {
			printf("[error] illegal memory access\n");
...
void x86_64_inc_jne_s(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t zf = bin[136] & 64;
	if (!zf) {
		if ((ip - k) >= bn || (ip - k) < 138) {
			printf("[error] illegal memory access\n");
...
void x86_64_inc_jne_u(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t zf = bin[136] & 64;
	if (!zf) {
		if ((ip + k) >= bn || (ip + k) < 138) {
			printf("[error] illegal memory access\n");
...
void x86_64_inc_jna_s(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
...
void x86_64_inc_jna_u(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t af = bin[136] & 16;
	if (!af) {
		if ((ip + k) >= bn || (ip + k) < 138) {
			printf("[error] illegal memory access\n");
...
void x86_64_inc_ja_s(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t af = bin[136] & 16;
	if (af) {
		if ((ip - k) >= bn || (ip - k) < 138) {
			printf("[error] illegal memory access\n");
...
void x86_64_inc_ja_u(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t af = bin[136] & 16;
	if (af) {
		if ((ip + k) >= bn || (ip + k) < 138) {
			printf("[error] illegal memory access\n");
...
void x86_64_inc_js_s(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
...
void x86_64_inc_js_u(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t sf = bin[136] & 128;
	if (sf) {
		if ((ip + k) >= bn || (ip + k) < 138) {
			printf("[error] illegal memory access\n");
...
void x86_64_inc_jns_s(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t sf = bin[136] & 128;
	if (!sf) {
		if ((ip - k) >= bn || (ip - k) < 138) {
			printf("[error] illegal memory access\n");
...
void x86_64_inc_jns_u(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t sf = bin[136] & 128;
	if (!sf) {
		if ((ip + k) >= bn || (ip + k) < 138) {
			printf("[error] illegal memory access\n");
...
void x86_64_inc_jpe_s(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
...
void x86_64_inc_jpe_u(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t pf = bin[136] & 4;
	if (pf) {
		if ((ip + k) >= bn || (ip + k) < 138) {
			printf("[error] illegal memory access\n");
...
void x86_64_inc_jpo_s(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t pf = bin[136] & 4;
	if (!pf) {
		if ((ip - k) >= bn || (ip - k) < 138) {
			printf("[error] illegal memory access\n");
...
void x86_64_inc_jpo_u(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t pf = bin[136] & 4;
	if (!pf) {
		if ((ip + k) >= bn || (ip + k) < 138) {
			printf("[error] illegal memory access\n");
...
void x86_64_inc_jl_s(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t sf = bin[136] & 128;
	uint8_t of = bin[137] & 8;
	if (sf ^ of) {
		if ((ip - k) >= bn || (ip - k) < 138) {
...
void x86_64_inc_jl_u(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t sf = bin[136] & 128;
	uint8_t of = bin[137] & 8;
	if (sf ^ of) {
		if ((ip + k) >= bn || (ip + k) < 138) {
...
void x86_64_inc_jge_s(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t sf = bin[136] & 128;
	uint8_t of = bin[137] & 8;
	if (!(sf ^ of)) {
		if ((ip - k) >= bn || (ip - k) < 138) {
...
void x86_64_inc_jge_u(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t sf = bin[136] & 128;
	uint8_t of = bin[137] & 8;
	if (!(sf ^ of)) {
		if ((ip + k) >= bn || (ip + k) < 138) {
...
void x86_64_inc_jle_s(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t zf = bin[136] & 64;
	uint8_t sf = bin[136] & 128;
	uint8_t of = bin[137] & 8;
	if (zf || (sf ^ of)) {
...
void x86_64_inc_jle_u(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t zf = bin[136] & 64;
	uint8_t sf = bin[136] & 128;
	uint8_t of = bin[137] & 8;
	if (zf || (sf ^ of)) {
...
void x86_64_inc_jg_s(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t zf = bin[136] & 64;
	uint8_t sf = bin[136] & 128;
	uint8_t of = bin[137] & 8;
	if (!zf && !(sf ^ of)) {
...
void x86_64_inc_jg_u(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint8_t zf = bin[136] & 64;
	uint8_t sf = bin[136] & 128;
	uint8_t of = bin[137] & 8;
	if (!zf && !(sf ^ of)) {
...
void x86_64_inc_jmp_s(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	if ((ip - k) >= bn || (ip - k) < 138) {
		printf("[error] illegal memory access\n");
		x86_64_reg_set_ip(bin, bn, ip);
	}
...
void x86_64_inc_jmp_u(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	if ((ip + k) >= bn || (ip + k) < 138) {
		printf("[error] illegal memory access\n");
		x86_64_reg_set_ip(bin, bn, ip);
	}
...
void x86_64_inc_call_s(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint64_t sp = x86_64_reg_get_sp(bin) + 138;
	if ((ip - k) >= bn || (ip - k) < 138 || (sp - 8) >= bn || (sp - 8) < 138) {
		printf("[error] illegal memory access\n");
		x86_64_reg_set_ip(bin, bn, ip);
...
void x86_64_inc_call_u(uint8_t* bin, uint64_t bn, uint64_t ip, uint32_t k) { 
	uint64_t sp = x86_64_reg_get_sp(bin) + 138;
	if ((ip + k) >= bn || (ip + k) < 138 || (sp - 8) >= bn || (sp - 8) < 138) {
		printf("[error] illegal memory access\n");
		x86_64_reg_set_ip(bin, bn, ip);
...
void x86_64_inc_ret(uint8_t* bin, uint64_t bn) {
	uint64_t sp = x86_64_reg_get_sp(bin) + 138;
	if (sp >= bn || sp < 138) {
		printf("[error] illegal memory access\n");
	}
	else {
		uint64_t ip = bin[sp] + (bin[sp + 1] << 8) + (bin[sp + 2] << 16) + (bin[sp + 3] << 24) + (bin[sp + 4] << 32) + (bin[sp + 5] << 40) + (bin[sp + 6] << 48) + (bin[sp + 7] << 56);
		sp = sp + 8;
		ip = ip + 138;
		x86_64_reg_set_ip(bin, bn, ip);
		x86_64_reg_set_sp(bin, sp);
	...
	if (bin[ip] == op0 && (bin[ip + 1] >> 3) == (0 | op1)) {
		...
		if ((mrd & 7) == 4) {
			...
			if (i == 4 && (b & 7) == 5) {
				...
				ip = ip + 1;
				if (lga) {
					x86_64_inc_op(bin, bn, x86_64_inc_addr_u32(bin, bn, b * 8, i * 8, 0, 0) + 138, k);
					x86_64_reg_set_ip(bin, bn, ip);
				}
				else {
					x86_64_inc_op(bin, bn, x86_64_inc_addr_u64(bin, bn, b * 8, i * 8, 0, 0) + 138, k);
					x86_64_reg_set_ip(bin, bn, ip);
				}
			}
		}
		...
		if ((mrd & 7) == 4) {
			...
			if (i == 4 && (b & 7) == 5) {
				...
				if (lgo) {
					...
					ip = ip + 2;
					if (lga) {
						x86_64_inc_op_16(bin, bn, x86_64_inc_addr_u32(bin, bn, b * 8, i * 8, 0, 0) + 138, k);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_16(bin, bn, x86_64_inc_addr_u64(bin, bn, b * 8, i * 8, 0, 0) + 138, k);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
				else if (!rx3) {
					...
					ip = ip + 4;
					if (lga) {
						x86_64_inc_op_32(bin, bn, x86_64_inc_addr_u32(bin, bn, b * 8, i * 8, 0, 0) + 138, k);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_32(bin, bn, x86_64_inc_addr_u64(bin, bn, b * 8, i * 8, 0, 0) + 138, k);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
				else {
					...
					ip = ip + 4;
					if (lga) {
						x86_64_inc_op_64(bin, bn, x86_64_inc_addr_u32(bin, bn, b * 8, i * 8, 0, 0) + 138, k);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_64(bin, bn, x86_64_inc_addr_u64(bin, bn, b * 8, i * 8, 0, 0) + 138, k);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
			}
		...
		if ((mrd & 7) == 4) {
			...
			if (d & 128) {
				...
				if (lgo) {
					...
					ip = ip + 2;
					if (lga) {
						x86_64_inc_op_16(bin, bn, x86_64_inc_addr_s32(bin, bn, b * 8, i * 8, s, d) + 138, k);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_16(bin, bn, x86_64_inc_addr_s64(bin, bn, b * 8, i * 8, s, d) + 138, k);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
				else if (!rx3) {
					...
					ip = ip + 4;
					if (lga) {
						x86_64_inc_op_32(bin, bn, x86_64_inc_addr_s32(bin, bn, b * 8, i * 8, s, d) + 138, k);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_32(bin, bn, x86_64_inc_addr_s64(bin, bn, b * 8, i * 8, s, d) + 138, k);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
				else {
					...
					ip = ip + 4;
					if (lga) {
						x86_64_inc_op_64(bin, bn, x86_64_inc_addr_s32(bin, bn, b * 8, i * 8, s, d) + 138, k);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_64(bin, bn, x86_64_inc_addr_s64(bin, bn, b * 8, i * 8, s, d) + 138, k);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
			}
				...
				if (lgo) {
					...
					ip = ip + 2;
					if (lga) {
						x86_64_inc_op_16(bin, bn, x86_64_inc_addr_u32(bin, bn, b * 8, i * 8, s, d) + 138, k);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_16(bin, bn, x86_64_inc_addr_u64(bin, bn, b * 8, i * 8, s, d) + 138, k);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
				else if (!rx3) {
					...
					ip = ip + 4;
					if (lga) {
						x86_64_inc_op_32(bin, bn, x86_64_inc_addr_u32(bin, bn, b * 8, i * 8, s, d) + 138, k);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_32(bin, bn, x86_64_inc_addr_u64(bin, bn, b * 8, i * 8, s, d) + 138, k);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
				else {
					...
					ip = ip + 4;
					if (lga) {
						x86_64_inc_op_64(bin, bn, x86_64_inc_addr_u32(bin, bn, b * 8, i * 8, s, d) + 138, k);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_64(bin, bn, x86_64_inc_addr_u64(bin, bn, b * 8, i * 8, s, d) + 138, k);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
			}	
...
uint8_t x86_64_inc_stck(uint8_t* bin, uint64_t bn, uint64_t ip, uint8_t op, void (*x86_64_inc_op_16) (uint8_t*, uint64_t, uint64_t), void (*x86_64_inc_op_64) (uint8_t*, uint64_t, uint64_t), uint8_t lga, uint8_t lgo, uint8_t rex, uint8_t rx0, uint8_t rx1, uint8_t rx2, uint8_t rx3) {
	if ((bin[ip] & 248) == op) {
...
uint8_t x86_64_inc_byt_imm(uint8_t* bin, uint64_t bn, uint64_t ip, uint8_t op, void (*x86_64_inc_op_s) (uint8_t*, uint64_t, uint64_t, uint32_t), void (*x86_64_inc_op_u) (uint8_t*, uint64_t, uint64_t, uint32_t), uint8_t lga, uint8_t lgo, uint8_t rex, uint8_t rx0, uint8_t rx1, uint8_t rx2, uint8_t rx3) {
	if (bin[ip] == op) {
		ip = ip + 1;
...
uint8_t x86_64_inc_cond(uint8_t* bin, uint64_t bn, uint64_t ip, uint8_t op, void (*x86_64_inc_op_s) (uint8_t*, uint64_t, uint64_t, uint32_t), void (*x86_64_inc_op_u) (uint8_t*, uint64_t, uint64_t, uint32_t), uint8_t lga, uint8_t lgo, uint8_t rex, uint8_t rx0, uint8_t rx1, uint8_t rx2, uint8_t rx3) {
	if (bin[ip] == 15 && bin[ip + 1] == op) {
		ip = ip + 2;
...
uint8_t x86_64_inc_jmp(uint8_t* bin, uint64_t bn, uint64_t ip, uint8_t op, void (*x86_64_inc_op_s) (uint8_t*, uint64_t, uint64_t, uint32_t), void (*x86_64_inc_op_u) (uint8_t*, uint64_t, uint64_t, uint32_t), uint8_t lga, uint8_t lgo, uint8_t rex, uint8_t rx0, uint8_t rx1, uint8_t rx2, uint8_t rx3) {
	if (bin[ip] == op) {
		ip = ip + 1;
...
uint8_t x86_64_inc_byt(uint8_t* bin, uint64_t bn, uint64_t ip, uint8_t op, void (*x86_64_inc_op) (uint8_t*, uint64_t), uint8_t lga, uint8_t lgo, uint8_t rex, uint8_t rx0, uint8_t rx1, uint8_t rx2, uint8_t rx3) {
	if (bin[ip] == op) {
		ip = ip + 1;
		x86_64_inc_op(bin, bn);
		return 0;
	}
	return 1;
}

uint8_t x86_64_inc_mvp(uint8_t* bin, uint64_t bn, uint64_t ip, uint8_t op, void (*x86_64_inc_op_8) (uint8_t*, uint64_t, uint64_t, uint8_t), void (*x86_64_inc_op_16) (uint8_t*, uint64_t, uint64_t, uint16_t), void (*x86_64_inc_op_32) (uint8_t*, uint64_t, uint64_t, uint32_t), void (*x86_64_inc_op_64) (uint8_t*, uint64_t, uint64_t, uint64_t), uint8_t lga, uint8_t lgo, uint8_t rex, uint8_t rx0, uint8_t rx1, uint8_t rx2, uint8_t rx3) {
	if ((bin[ip] & 248) == op) {
		uint8_t reg = (bin[ip] & 7) + (8 * rx0);
		ip = ip + 1;
		...
		ip = ip + 1;
		if (rex) {
			x86_64_inc_op_8(bin, bn, reg * 8, k);
			x86_64_reg_set_ip(bin, bn, ip);
		}
		else {
			x86_64_inc_op_8(bin, bn, ((reg & 3) * 8) + !!(reg & 4), k);
			x86_64_reg_set_ip(bin, bn, ip);
		}
		return 0;
	}
	else if ((bin[ip] & 248) == op + 8) {
		uint8_t reg = (bin[ip] & 7) + (8 * rx0);
		ip = ip + 1;
		...
		if (lgo) {
			uint16_t k = bin[ip] + (bin[ip + 1] << 8);
			ip = ip + 2;
			x86_64_inc_op_16(bin, bn, reg * 8, k);
			x86_64_reg_set_ip(bin, bn, ip);
		}
		else if (!rx3) {
			uint32_t k = bin[ip] + (bin[ip + 1] << 8) + (bin[ip + 2] << 16) + (bin[ip + 3] << 24);
			...
			uint64_t k = bin[ip] + (bin[ip + 1] << 8) + (bin[ip + 2] << 16) + (bin[ip + 3] << 24) + (bin[ip + 4] << 32) + (bin[ip + 5] << 40) + (bin[ip + 6] << 48) + (bin[ip + 7] << 56);
			ip = ip + 8;
			x86_64_inc_op_8(bin, bn, reg * 8, k);
			x86_64_reg_set_ip(bin, bn, ip);
		}
...
uint8_t x86_64_inc_blnk(uint8_t* bin, uint64_t bn, uint64_t ip, uint8_t op0, uint8_t op1, void (*x86_64_inc_op_8) (uint8_t*, uint64_t, uint64_t), void (*x86_64_inc_op_16) (uint8_t*, uint64_t, uint64_t), void (*x86_64_inc_op_32) (uint8_t*, uint64_t, uint64_t), void (*x86_64_inc_op_64) (uint8_t*, uint64_t, uint64_t), uint8_t lga, uint8_t lgo, uint8_t rex, uint8_t rx0, uint8_t rx1, uint8_t rx2, uint8_t rx3) {
	if (bin[ip] == op0 && (bin[ip + 1] >> 3) == (0 | op1)) {
		ip = ip + 1;
		uint8_t mrd = (bin[ip] & 7) + (8 * rx0);
		...
		if ((mrd & 7) == 4) {
			...
			if (i == 4 && (b & 7) == 5) {
				...
				ip = ip + 1;
				if (lga) {
					x86_64_inc_op_8(bin, bn, x86_64_inc_addr_u32(bin, bn, b * 8, i * 8, s, 0) + 138);
					x86_64_reg_set_ip(bin, bn, ip);
				}
				else {
					x86_64_inc_op_8(bin, bn, x86_64_inc_addr_u64(bin, bn, b * 8, i * 8, s, 0) + 138);
					x86_64_reg_set_ip(bin, bn, ip);
				}
			}
		}
			...
			if (d & 2147483648) {
				d = ~d + 1;
				if (lga) {
					x86_64_inc_op_8(bin, bn, ((uint32_t) ip) - d);
					x86_64_reg_set_ip(bin, bn, ip);
				}
				else {
					x86_64_inc_op_8(bin, bn, ip - d);
					x86_64_reg_set_ip(bin, bn, ip);
				}
			}
			else {
			...
			ip = ip + 1;
			if (lga) {
				x86_64_inc_op_8(bin, bn, x86_64_inc_addr_u32(bin, bn, mrd * 8, 32, 0, 0) + 138);
				x86_64_reg_set_ip(bin, bn, ip);
			}
			else {
				x86_64_inc_op_8(bin, bn, x86_64_inc_addr_u64(bin, bn, mrd * 8, 32, 0, 0) + 138);
				x86_64_reg_set_ip(bin, bn, ip);
			}
		}
		return 0;
		...
		if ((mrd & 7) == 4) {
			...
			if (d & 128) {
				d = ~d + 1;
				if (lga) {
					x86_64_inc_op_8(bin, bn, x86_64_inc_addr_s32(bin, bn, b * 8, i * 8, s, d) + 138);
					x86_64_reg_set_ip(bin, bn, ip);
				}
				else {
					x86_64_inc_op_8(bin, bn, x86_64_inc_addr_s64(bin, bn, b * 8, i * 8, s, d) + 138);
					x86_64_reg_set_ip(bin, bn, ip);
				}
			}
			else {
				if (lga) {
					x86_64_inc_op_8(bin, bn, x86_64_inc_addr_u32(bin, bn, b * 8, i * 8, s, d) + 138);
					x86_64_reg_set_ip(bin, bn, ip);
				}
				else {
					x86_64_inc_op_8(bin, bn, x86_64_inc_addr_u64(bin, bn, b * 8, i * 8, s, d) + 138);
					x86_64_reg_set_ip(bin, bn, ip);
				}
			}
		}
			...
			if (d & 128) {
				d = ~d + 1;
				if (lga) {
					x86_64_inc_op_8(bin, bn, x86_64_inc_addr_s32(bin, bn, mrd * 8, 32, 0, d) + 138);
					x86_64_reg_set_ip(bin, bn, ip);
				}
				else {
					x86_64_inc_op_8(bin, bn, x86_64_inc_addr_s64(bin, bn, mrd * 8, 32, 0, d) + 138);
					x86_64_reg_set_ip(bin, bn, ip);
				}
			}
			else {
				if (lga) {
					x86_64_inc_op_8(bin, bn, x86_64_inc_addr_u32(bin, bn, mrd * 8, 32, 0, d) + 138);
					x86_64_reg_set_ip(bin, bn, ip);
				}
				else {
					x86_64_inc_op_8(bin, bn, x86_64_inc_addr_u64(bin, bn, mrd * 8, 32, 0, d) + 138);
					x86_64_reg_set_ip(bin, bn, ip);
				}
			}
		}
		...
		ip = ip + 1;
		if (rex) {
			x86_64_inc_op_8(bin, bn, mrd * 8);
			x86_64_reg_set_ip(bin, bn, ip);
		}
		else {
			x86_64_inc_op_8(bin, bn, ((mrd & 3) * 8) + !!(mrd & 4));
			x86_64_reg_set_ip(bin, bn, ip);
		}
		return 0;
	}
	else if (bin[ip] == op0 + 1 && (bin[ip + 1] >> 3) == (0 | op1)) {
		ip = ip + 1;
		uint8_t mrd = (bin[ip] & 7) + (8 * rx0);
		...
		if ((mrd & 7) == 4) {
			...
			if (i == 4 && (b & 7) == 5) {
				...
				if (lgo) {
					if (lga) {
						x86_64_inc_op_16(bin, bn, x86_64_inc_addr_u32(bin, bn, b * 8, i * 8, s, 0) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_16(bin, bn, x86_64_inc_addr_u64(bin, bn, b * 8, i * 8, s, 0) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
				else if (!rx3) {
					if (lga) {
						x86_64_inc_op_32(bin, bn, x86_64_inc_addr_u32(bin, bn, b * 8, i * 8, s, 0) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_32(bin, bn, x86_64_inc_addr_u64(bin, bn, b * 8, i * 8, s, 0) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
				else {
					if (lga) {
						x86_64_inc_op_64(bin, bn, x86_64_inc_addr_u32(bin, bn, b * 8, i * 8, s, 0) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_64(bin, bn, x86_64_inc_addr_u64(bin, bn, b * 8, i * 8, s, 0) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
			}
			...
			if (d & 2147483648) {
				...
				if (lgo) {
					if (lga) {
						x86_64_inc_op_16(bin, bn, ((uint32_t) ip) - d);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_16(bin, bn, ip - d);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
				else if (!rx3) {
					if (lga) {
						x86_64_inc_op_32(bin, bn, ((uint32_t) ip) - d);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_32(bin, bn, ip - d);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
				else {
					if (lga) {
						x86_64_inc_op_64(bin, bn, ((uint32_t) ip) - d);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_64(bin, bn, ip - d);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
			}
			...
			if (lgo) {
				if (lga) {
					x86_64_inc_op_16(bin, bn, x86_64_inc_addr_u32(bin, bn, mrd * 8, 32, 0, 0) + 138);
					x86_64_reg_set_ip(bin, bn, ip);
				}
				else {
					x86_64_inc_op_16(bin, bn, x86_64_inc_addr_u64(bin, bn, mrd * 8, 32, 0, 0) + 138);
					x86_64_reg_set_ip(bin, bn, ip);
				}
			}
			else if (!rx3) {
				if (lga) {
					x86_64_inc_op_32(bin, bn, x86_64_inc_addr_u32(bin, bn, mrd * 8, 32, 0, 0) + 138);
					x86_64_reg_set_ip(bin, bn, ip);
				}
				else {
					x86_64_inc_op_32(bin, bn, x86_64_inc_addr_u64(bin, bn, mrd * 8, 32, 0, 0) + 138);
					x86_64_reg_set_ip(bin, bn, ip);
				}
			}
			else {
				if (lga) {
					x86_64_inc_op_64(bin, bn, x86_64_inc_addr_u32(bin, bn, mrd * 8, 32, 0, 0) + 138);
					x86_64_reg_set_ip(bin, bn, ip);
				}
				else {
					x86_64_inc_op_64(bin, bn, x86_64_inc_addr_u64(bin, bn, mrd * 8, 32, 0, 0) + 138);
					x86_64_reg_set_ip(bin, bn, ip);
				}
			}
		}
	...
	else if (bin[ip] == op0 + 1 && (bin[ip + 1] >> 3) == (8 | op1)) {
		ip = ip + 1;
		uint8_t mrd = (bin[ip] & 7) + (8 * rx0);
		...
		if ((mrd & 7) == 4) {
			...
			if (d & 128) {
				...
				if (lgo) {
					if (lga) {
						x86_64_inc_op_16(bin, bn, x86_64_inc_addr_s32(bin, bn, b * 8, i * 8, s, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_16(bin, bn, x86_64_inc_addr_s64(bin, bn, b * 8, i * 8, s, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
				else if (!rx3) {
					if (lga) {
						x86_64_inc_op_32(bin, bn, x86_64_inc_addr_s32(bin, bn, b * 8, i * 8, s, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_32(bin, bn, x86_64_inc_addr_s64(bin, bn, b * 8, i * 8, s, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
				else {
					if (lga) {
						x86_64_inc_op_64(bin, bn, x86_64_inc_addr_s32(bin, bn, b * 8, i * 8, s, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_64(bin, bn, x86_64_inc_addr_s64(bin, bn, b * 8, i * 8, s, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
			}
				...
				if (lgo) {
					if (lga) {
						x86_64_inc_op_16(bin, bn, x86_64_inc_addr_u32(bin, bn, b * 8, i * 8, s, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_16(bin, bn, x86_64_inc_addr_u64(bin, bn, b * 8, i * 8, s, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
				else if (!rx3) {
					if (lga) {
						x86_64_inc_op_32(bin, bn, x86_64_inc_addr_u32(bin, bn, b * 8, i * 8, s, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_32(bin, bn, x86_64_inc_addr_u64(bin, bn, b * 8, i * 8, s, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
				else {
					if (lga) {
						x86_64_inc_op_64(bin, bn, x86_64_inc_addr_u32(bin, bn, b * 8, i * 8, s, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_64(bin, bn, x86_64_inc_addr_u64(bin, bn, b * 8, i * 8, s, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
			}
			...
			if (d & 128) {
				...
				if (lgo) {
					if (lga) {
						x86_64_inc_op_16(bin, bn, x86_64_inc_addr_s32(bin, bn, mrd * 8, 32, 0, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_16(bin, bn, x86_64_inc_addr_s64(bin, bn, mrd * 8, 32, 0, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
				else if (!rx3) {
					if (lga) {
						x86_64_inc_op_32(bin, bn, x86_64_inc_addr_s32(bin, bn, mrd * 8, 32, 0, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_32(bin, bn, x86_64_inc_addr_s64(bin, bn, mrd * 8, 32, 0, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
				else {
					if (lga) {
						x86_64_inc_op_64(bin, bn, x86_64_inc_addr_s32(bin, bn, mrd * 8, 32, 0, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_64(bin, bn, x86_64_inc_addr_s64(bin, bn, mrd * 8, 32, 0, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
			}
				...
				if (lgo) {
					if (lga) {
						x86_64_inc_op_16(bin, bn, x86_64_inc_addr_u32(bin, bn, mrd * 8, 32, 0, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_16(bin, bn, x86_64_inc_addr_u64(bin, bn, mrd * 8, 32, 0, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
				else if (!rx3) {
					if (lga) {
						x86_64_inc_op_32(bin, bn, x86_64_inc_addr_u32(bin, bn, mrd * 8, 32, 0, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_32(bin, bn, x86_64_inc_addr_u64(bin, bn, mrd * 8, 32, 0, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
				else {
					if (lga) {
						x86_64_inc_op_64(bin, bn, x86_64_inc_addr_u32(bin, bn, mrd * 8, 32, 0, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
					else {
						x86_64_inc_op_64(bin, bn, x86_64_inc_addr_u64(bin, bn, mrd * 8, 32, 0, d) + 138);
						x86_64_reg_set_ip(bin, bn, ip);
					}
				}
			}
	...
	else if (bin[ip] == op0 + 1 && (bin[ip + 1] >> 3) == (16 | op1)) {
		ip = ip + 1;
		uint8_t mrd = (bin[ip] & 7) + (8 * rx0);
	...
	else if (bin[ip] == op0 + 1 && (bin[ip + 1] >> 3) == (24 | op1)) {
		ip = ip + 1;
		uint8_t mrd = (bin[ip] & 7) + (8 * rx0);
		
		ip = ip + 1;
		if (lgo) {
			x86_64_inc_op_16(bin, bn, mrd * 8);
			x86_64_reg_set_ip(bin, bn, ip);
		}
		else if (!rx3) {
			x86_64_inc_op_32(bin, bn, mrd * 8);
			x86_64_reg_set_ip(bin, bn, ip);
		}
		else {
			x86_64_inc_op_64(bin, bn, mrd * 8);
			x86_64_reg_set_ip(bin, bn, ip);
		}
		return 0;
	}
...
uint8_t x86_64_inc_clnk(uint8_t* bin, uint64_t bn, uint64_t ip, uint8_t op0, uint8_t op1, void (*x86_64_inc_op_16) (uint8_t*, uint64_t, uint64_t, uint64_t), void (*x86_64_inc_op_64) (uint8_t*, uint64_t, uint64_t, uint64_t), uint8_t lga, uint8_t lgo, uint8_t rex, uint8_t rx0, uint8_t rx1, uint8_t rx2, uint8_t rx3) {
	if (bin[ip] == op0 && (bin[ip + 1] >> 3) == (0 | op1)) {
		ip = ip + 1;
		uint8_t mrd = (bin[ip] & 7) + (8 * rx0);
		...
		if ((mrd & 7) == 4) {
			...
			if (i == 4 && (b & 7) == 5) {
				...
				if (lgo) {
					if (lga) {
						x86_64_inc_op_16(bin, bn, ip, x86_64_inc_addr_u32(bin, bn, b * 8, i * 8, s, 0) + 138);
					}
					else {
						x86_64_inc_op_16(bin, bn, ip, x86_64_inc_addr_u64(bin, bn, b * 8, i * 8, s, 0) + 138);
					}
				}
				else {
					if (lga) {
						x86_64_inc_op_64(bin, bn, ip, x86_64_inc_addr_u32(bin, bn, b * 8, i * 8, s, 0) + 138);
					}
					else {
						x86_64_inc_op_64(bin, bn, ip, x86_64_inc_addr_u64(bin, bn, b * 8, i * 8, s, 0) + 138);
					}
				}
			}
		}
			...
			if (d & 2147483648) {
				...
				if (lgo) {
					if (lga) {
						x86_64_inc_op_16(bin, bn, ip, ((uint32_t) ip) - d);
					}
					else {
						x86_64_inc_op_16(bin, bn, ip, ip - d);
					}
				}
				else {
					if (lga) {
						x86_64_inc_op_64(bin, bn, ip, ((uint32_t) ip) - d);
					}
					else {
						x86_64_inc_op_64(bin, bn, ip, ip - d);
					}
				}
			}
			else {
			...
			if (lgo) {
				if (lga) {
					x86_64_inc_op_16(bin, bn, ip, x86_64_inc_addr_u32(bin, bn, mrd * 8, 32, 0, 0) + 138);
				}
				else {
					x86_64_inc_op_16(bin, bn, ip, x86_64_inc_addr_u64(bin, bn, mrd * 8, 32, 0, 0) + 138);
				}
			}
			else {
				if (lga) {
					x86_64_inc_op_64(bin, bn, ip, x86_64_inc_addr_u32(bin, bn, mrd * 8, 32, 0, 0) + 138);
				}
				else {
					x86_64_inc_op_64(bin, bn, ip, x86_64_inc_addr_u64(bin, bn, mrd * 8, 32, 0, 0) + 138);
				}
			}
		}
		return 0;
		...
		if ((mrd & 7) == 4) {
			...
			if (d & 128) {
				...
				if (lgo) {
					if (lga) {
						x86_64_inc_op_16(bin, bn, ip, x86_64_inc_addr_s32(bin, bn, b * 8, i * 8, s, d) + 138);
					}
					else {
						x86_64_inc_op_16(bin, bn, ip, x86_64_inc_addr_s64(bin, bn, b * 8, i * 8, s, d) + 138);
					}
				}
				else {
					if (lga) {
						x86_64_inc_op_64(bin, bn, ip, x86_64_inc_addr_s32(bin, bn, b * 8, i * 8, s, d) + 138);
					}
					else {
						x86_64_inc_op_64(bin, bn, ip, x86_64_inc_addr_s64(bin, bn, b * 8, i * 8, s, d) + 138);
					}
				}
			}
			else {
				if (lgo) {
					if (lga) {
						x86_64_inc_op_16(bin, bn, ip, x86_64_inc_addr_u32(bin, bn, b * 8, i * 8, s, d) + 138);
					}
					else {
						x86_64_inc_op_16(bin, bn, ip, x86_64_inc_addr_u64(bin, bn, b * 8, i * 8, s, d) + 138);
					}
				}
				else {
					if (lga) {
						x86_64_inc_op_64(bin, bn, ip, x86_64_inc_addr_u32(bin, bn, b * 8, i * 8, s, d) + 138);
					}
					else {
						x86_64_inc_op_64(bin, bn, ip, x86_64_inc_addr_u64(bin, bn, b * 8, i * 8, s, d) + 138);
					}
				}
			}
		}
			...
			if (d & 128) {
				...
				if (lgo) {
					if (lga) {
						x86_64_inc_op_16(bin, bn, ip, x86_64_inc_addr_s32(bin, bn, mrd * 8, 32, 0, d) + 138);
					}
					else {
						x86_64_inc_op_16(bin, bn, ip, x86_64_inc_addr_s64(bin, bn, mrd * 8, 32, 0, d) + 138);
					}
				}
				else {
					if (lga) {
						x86_64_inc_op_64(bin, bn, ip, x86_64_inc_addr_s32(bin, bn, mrd * 8, 32, 0, d) + 138);
					}
					else {
						x86_64_inc_op_64(bin, bn, ip, x86_64_inc_addr_s64(bin, bn, mrd * 8, 32, 0, d) + 138);
					}
				}
			}
			else {
				if (lgo) {
					if (lga) {
						x86_64_inc_op_16(bin, bn, ip, x86_64_inc_addr_u32(bin, bn, mrd * 8, 32, 0, d) + 138);
					}
					else {
						x86_64_inc_op_16(bin, bn, ip, x86_64_inc_addr_u64(bin, bn, mrd * 8, 32, 0, d) + 138);
					}
				}
				else {
					if (lga) {
						x86_64_inc_op_64(bin, bn, ip, x86_64_inc_addr_u32(bin, bn, mrd * 8, 32, 0, d) + 138);
					}
					else {
						x86_64_inc_op_64(bin, bn, ip, x86_64_inc_addr_u64(bin, bn, mrd * 8, 32, 0, d) + 138);
					}
				}
			}
		}
		...
		ip = ip + 1;
		if (lgo) {
			x86_64_inc_op_16(bin, bn, ip, mrd * 8);
		}
		else {
			x86_64_inc_op_64(bin, bn, ip, mrd * 8);
		}
		return 0;
	}
	return 1;
...
void x86_64_inc(uint8_t* bin, uint64_t bn, uint64_t ip, void (*x86_64_syscall) (uint8_t*, uint64_t)) {
	uint8_t lga = 0;
	uint8_t lgo = 0;
	uint8_t rex = 0;
	uint8_t rx0 = 0;
	...
	if (eo) {
		eo = x86_64_inc_byt_imm(bin, bn, ip, 112, x86_64_inc_jo_s, x86_64_inc_jo_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_byt_imm(bin, bn, ip, 113, x86_64_inc_jno_s, x86_64_inc_jno_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_byt_imm(bin, bn, ip, 114, x86_64_inc_jc_s, x86_64_inc_jc_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_byt_imm(bin, bn, ip, 115, x86_64_inc_jnc_s, x86_64_inc_jnc_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_byt_imm(bin, bn, ip, 116, x86_64_inc_je_s, x86_64_inc_je_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_byt_imm(bin, bn, ip, 117, x86_64_inc_jne_s, x86_64_inc_jne_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_byt_imm(bin, bn, ip, 118, x86_64_inc_jna_s, x86_64_inc_jna_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_byt_imm(bin, bn, ip, 119, x86_64_inc_ja_s, x86_64_inc_ja_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_byt_imm(bin, bn, ip, 120, x86_64_inc_js_s, x86_64_inc_js_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_byt_imm(bin, bn, ip, 121, x86_64_inc_jns_s, x86_64_inc_jns_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_byt_imm(bin, bn, ip, 122, x86_64_inc_jpe_s, x86_64_inc_jpe_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_byt_imm(bin, bn, ip, 123, x86_64_inc_jpo_s, x86_64_inc_jpo_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_byt_imm(bin, bn, ip, 124, x86_64_inc_jl_s, x86_64_inc_jl_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_byt_imm(bin, bn, ip, 125, x86_64_inc_jge_s, x86_64_inc_jge_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_byt_imm(bin, bn, ip, 126, x86_64_inc_jle_s, x86_64_inc_jle_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_byt_imm(bin, bn, ip, 127, x86_64_inc_jg_s, x86_64_inc_jg_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
	...
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 143, 0, 0, x86_64_inc_pop_reg_16, 0, x86_64_inc_pop_reg_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_mvp(bin, bn, ip, 176, x86_64_inc_mov_imm_8, x86_64_inc_mov_imm_16, x86_64_inc_mov_imm_32, x86_64_inc_mov_imm_64_mvp, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_k80(bin, bn, ip, 192, 0, x86_64_inc_rol_imm_8, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_k80(bin, bn, ip, 192, 1, x86_64_inc_ror_imm_8, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_k80(bin, bn, ip, 192, 2, x86_64_inc_rcl_imm_8, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_k80(bin, bn, ip, 192, 3, x86_64_inc_rcr_imm_8, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_k80(bin, bn, ip, 192, 4, x86_64_inc_shl_imm_8, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_k80(bin, bn, ip, 192, 5, x86_64_inc_shr_imm_8, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_k80(bin, bn, ip, 192, 6, x86_64_inc_sal_imm_8, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_k80(bin, bn, ip, 192, 7, x86_64_inc_sar_imm_8, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_k81(bin, bn, ip, 193, 0, x86_64_inc_rol_imm_16, x86_64_inc_rol_imm_32, x86_64_inc_rol_imm_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_k81(bin, bn, ip, 193, 1, x86_64_inc_ror_imm_16, x86_64_inc_ror_imm_32, x86_64_inc_ror_imm_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_k81(bin, bn, ip, 193, 2, x86_64_inc_rcl_imm_16, x86_64_inc_rcl_imm_32, x86_64_inc_rcl_imm_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_k81(bin, bn, ip, 193, 3, x86_64_inc_rcr_imm_16, x86_64_inc_rcr_imm_32, x86_64_inc_rcr_imm_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_k81(bin, bn, ip, 193, 4, x86_64_inc_shl_imm_16, x86_64_inc_shl_imm_32, x86_64_inc_shl_imm_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_k81(bin, bn, ip, 193, 5, x86_64_inc_shr_imm_16, x86_64_inc_shr_imm_32, x86_64_inc_shr_imm_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_k81(bin, bn, ip, 193, 6, x86_64_inc_sal_imm_16, x86_64_inc_sal_imm_32, x86_64_inc_sal_imm_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_k81(bin, bn, ip, 193, 7, x86_64_inc_sar_imm_16, x86_64_inc_sar_imm_32, x86_64_inc_sar_imm_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_byt(bin, bn, ip, 195, x86_64_inc_ret, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_k80(bin, bn, ip, 198, 0, x86_64_inc_mov_imm_8, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_imm(bin, bn, ip, 199, 0, x86_64_inc_mov_imm_16, x86_64_inc_mov_imm_32, x86_64_inc_mov_imm_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 208, 0, x86_64_inc_rol_k1_8, x86_64_inc_rol_k1_16, x86_64_inc_rol_k1_32, x86_64_inc_rol_k1_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 208, 1, x86_64_inc_ror_k1_8, x86_64_inc_ror_k1_16, x86_64_inc_ror_k1_32, x86_64_inc_ror_k1_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 208, 2, x86_64_inc_rcl_k1_8, x86_64_inc_rcl_k1_16, x86_64_inc_rcl_k1_32, x86_64_inc_rcl_k1_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 208, 3, x86_64_inc_rcr_k1_8, x86_64_inc_rcr_k1_16, x86_64_inc_rcr_k1_32, x86_64_inc_rcr_k1_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 208, 4, x86_64_inc_shl_k1_8, x86_64_inc_shl_k1_16, x86_64_inc_shl_k1_32, x86_64_inc_shl_k1_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 208, 5, x86_64_inc_shr_k1_8, x86_64_inc_shr_k1_16, x86_64_inc_shr_k1_32, x86_64_inc_shr_k1_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 208, 6, x86_64_inc_sal_k1_8, x86_64_inc_sal_k1_16, x86_64_inc_sal_k1_32, x86_64_inc_sal_k1_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 208, 7, x86_64_inc_sar_k1_8, x86_64_inc_sar_k1_16, x86_64_inc_sar_k1_32, x86_64_inc_sar_k1_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 210, 0, x86_64_inc_rol_cl_8, x86_64_inc_rol_cl_16, x86_64_inc_rol_cl_32, x86_64_inc_rol_cl_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 210, 1, x86_64_inc_ror_cl_8, x86_64_inc_ror_cl_16, x86_64_inc_ror_cl_32, x86_64_inc_ror_cl_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 210, 2, x86_64_inc_rcl_cl_8, x86_64_inc_rcl_cl_16, x86_64_inc_rcl_cl_32, x86_64_inc_rcl_cl_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 210, 3, x86_64_inc_rcr_cl_8, x86_64_inc_rcr_cl_16, x86_64_inc_rcr_cl_32, x86_64_inc_rcr_cl_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 210, 4, x86_64_inc_shl_cl_8, x86_64_inc_shl_cl_16, x86_64_inc_shl_cl_32, x86_64_inc_shl_cl_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 210, 5, x86_64_inc_shr_cl_8, x86_64_inc_shr_cl_16, x86_64_inc_shr_cl_32, x86_64_inc_shr_cl_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 210, 6, x86_64_inc_sal_cl_8, x86_64_inc_sal_cl_16, x86_64_inc_sal_cl_32, x86_64_inc_sal_cl_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 210, 7, x86_64_inc_sar_cl_8, x86_64_inc_sar_cl_16, x86_64_inc_sar_cl_32, x86_64_inc_sar_cl_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_jmp(bin, bn, ip, 232, x86_64_inc_call_s, x86_64_inc_call_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
	...
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 246, 3, x86_64_inc_neg_reg_8, x86_64_inc_neg_reg_16, x86_64_inc_neg_reg_32, x86_64_inc_neg_reg_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 254, 0, x86_64_inc_inc_reg_8, x86_64_inc_inc_reg_16, x86_64_inc_inc_reg_32, x86_64_inc_inc_reg_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_blnk(bin, bn, ip, 254, 1, x86_64_inc_dec_reg_8, x86_64_inc_dec_reg_16, x86_64_inc_dec_reg_32, x86_64_inc_dec_reg_64, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
	...
	if (eo) {
		eo = x86_64_inc_cond(bin, bn, ip, 128, x86_64_inc_jo_s, x86_64_inc_jo_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_cond(bin, bn, ip, 129, x86_64_inc_jno_s, x86_64_inc_jno_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_cond(bin, bn, ip, 130, x86_64_inc_jc_s, x86_64_inc_jc_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_cond(bin, bn, ip, 131, x86_64_inc_jnc_s, x86_64_inc_jnc_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_cond(bin, bn, ip, 132, x86_64_inc_je_s, x86_64_inc_je_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_cond(bin, bn, ip, 133, x86_64_inc_jne_s, x86_64_inc_jne_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_cond(bin, bn, ip, 134, x86_64_inc_jna_s, x86_64_inc_jna_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_cond(bin, bn, ip, 135, x86_64_inc_ja_s, x86_64_inc_ja_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_cond(bin, bn, ip, 136, x86_64_inc_js_s, x86_64_inc_js_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_cond(bin, bn, ip, 137, x86_64_inc_jns_s, x86_64_inc_jns_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_cond(bin, bn, ip, 138, x86_64_inc_jpe_s, x86_64_inc_jpe_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_cond(bin, bn, ip, 139, x86_64_inc_jpo_s, x86_64_inc_jpo_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_cond(bin, bn, ip, 140, x86_64_inc_jl_s, x86_64_inc_jl_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_cond(bin, bn, ip, 141, x86_64_inc_jge_s, x86_64_inc_jge_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_cond(bin, bn, ip, 142, x86_64_inc_jle_s, x86_64_inc_jle_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
		eo = x86_64_inc_cond(bin, bn, ip, 143, x86_64_inc_jg_s, x86_64_inc_jg_u, lga, lgo, rex, rx0, rx1, rx2, rx3);
	}
	if (eo) {
